...
def fix_label(w):
    return "".join(map(lambda x: DICT[x.tobytes()], w))


def fix_word_input(words):
...
# this is MUTABLE! I hate it, but otherwise is so slow
...
class RNNExplorerWithInternalState(object):
    ...
    def __getitem__(self, words):
        ...
        # new_states = list(itertools.chain.from_iterable(new_states))
...
def length(data):
    ...
    #   of each sequence
    # """
    data = tf.slice(data, [0,0,0], [-1,-1, 4])
...
class RNNStateExplorer(object):
    ...
    def _lstm_model_fn(self, features):
        ...
        lengths = length(features[KEY_IN_DATA])
